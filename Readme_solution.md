...
### Step 1: Create dedicated VPC and whole components
    ...
    - Create VPC. 
        ...
        no ipv6 CIDR block
        tenancy: default
    - select `aws_capstone-VPC` VPC, click `Actions` and `enable DNS hostnames` for the `aws_capstone-VPC`. # bunu unutma
    ...
    - capstone-endpoint
    - service name  : `com.amazonaws.us-east-1.s3` and select Gateway 
    - VPC           : `aws_capstone-VPC`
    - Route Table   : private route tables
    - Policy        : `Full Access`
    - Create
    - go to the private RT, see the endpoint pl-63a5400a
...
- Click Create Bucket
```text
Bucket Name : awscapstoneadamblog
Region      : N.Virginia
Object Ownership: ACLs enabled 
Block all public access : Unchecked (check kaldırıyoruz)
check I acknowledge ....
...
- Click Create Bucket
```text
Bucket Name : www.awsdevopsalex.click    <YOUR DNS NAME>
Region      : N.Virginia
Object Ownership: ACLs enabled 
Block all public access : Unchecked (check kaldırıyoruz)
check I acknowledge ....
...
- Selects created `www.awsdevopsalex.click <YOUR DNS NAME>` bucket ---> Properties ---> Static website hosting
```text 
...
save changes
```
- Select `www.awsdevopsalex.click <YOUR DNS NAME>` bucket ---> select Upload and upload `index.html` and `sorry.jpg` files from given folder---> Permissions ---> Grant public-read access ---> Checked warning massage after click upload

- Select `www.awsdevopsalex.click <YOUR DNS NAME>` bucket---> Properties---> select Static website hosting and go to the browser(Failover Scenario)

## Step 5: Copy files downloaded or cloned from `Clarusway_project` repo on Github 
src/cblog/setting.py/85-92 and 146 line(awscapstonealexblog /s3 bucket name) and 148 "us-east-1" update
open .env and update password=Clarusway1234

## Step 6: Prepair your Github repository
- Create private project repository on your Github and clone it on your local. Copy all files and folders which are downloaded from clarusway repo under this folder. 
- Commit and push them on your private Git hup Repo.

## Step 7: Prepare a userdata to be utilized in Launch Template
Please  update line 6-7-12  in userdata.sh
```bash
#!/bin/bash
apt-get update -y
...
git clone https://$TOKEN@<YOUR PRIVATE REPO URL>github.com/alexanderadam2022/project-503_v2.git
...
!!!IMPORTANT!!!
- select newly created NAT instance and 
- stop source/destination check #### ÇOK ÖNEMLİİİİİİİ
- go to private route table and write a rule
- edit rules and add 3 route below;
```
...
```
- click create
HTTP and HTTPS listener after best practice execute;
...
``` -->
go to the ec2 load başancer-->alb adress DNS name copy adress and see the adress in your browser
...
Alternate Domain Names  add item        : www.awsdevopsalex.click
SSL Certificate                         : Custom SSL Certificate (example.com) ---> *.awsdevopsalex.click
Other stuff                             : Keep them as are                  
```
##open clontfront distributions adress--> d3pqc4........net  in your browser
...
Protocol            : HTTP
Domain Name         : d3pqc4........net (Write cloudfront domain name whithout https://)
Port                : 80
Path                : leave it blank
Other stuff         : Keep them as are
```
...
Runtime                 : Python 3.8
Permission use an existing role : capstonelambda
